...
def google_speech_sentences(row, df):
    #Lower quality but has free tier
...
def google_cloud_tts(row, df):
    #Higher quality but is paid
    ...
    #MUST INCLUDE THE BELOW STATEMENTS FOR PROPER FUNCTIONING
    #import os
    #from google.cloud import texttospeech
    #os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = "/Users/casey/Documents/PythonProjects/Spanish Learning App/adelante-app.json"
...
def create_sentences_from_vocab(vocab_df):
    try:
        ...
        #Audio generation and saving of DataFrame as CSV, using LOWER QUALITY Google TTS API
        ...
        #Audio generation and saving of DataFrame as CSV, using HIGHER QUALITY Google Cloud TTS API
        sentences_df.loc[:,'Audio'] = sentences_df.apply(google_cloud_tts, df=sentences_df, axis=1)
        sentences_df['Audio'] = sentences_df['Audio'].str.replace("/Users/casey/Documents/PythonProjects/Spanish Learning App", ".")
        sentences_df.to_csv(sentence_csv_path + week_selection + ".csv")
...
#Google Cloud TTS API and client
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = "/Users/casey/Documents/PythonProjects/Spanish Learning App/adelante-app.json"
client = texttospeech.TextToSpeechClient()
...
try:
    ...
    with tab1:
        with st.form("data_selection_form"):
            ...
            with data_column3:
                week_selection = st.text_input("Week or topic:")
            with data_column4:
                st.markdown(" ")
                st.text("")
